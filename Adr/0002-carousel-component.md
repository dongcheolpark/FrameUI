...
### 3.1. 포인터 드래그 (마우스 · 터치 · 펜 통합)

- 드래그는 **Pointer Events** (`pointerdown`/`pointermove`/`pointerup`/`pointercancel`)로 통일합니다. Touch/Mouse 이벤트를 각각 처리하지 않습니다 — iOS Safari 포함 최신 브라우저는 Pointer Events를 지원하며, 마우스·터치·펜을 일관된 API로 다룰 수 있습니다.
- **데스크톱 마우스 드래그도 1급 상호작용으로 지원**합니다. `pointerType === "mouse"` 여부에 따라 로직을 분기하지 않습니다. `setPointerCapture`를 사용해 커서가 Viewport 바깥으로 벗어나도 드래그 추적을 유지합니다. 다만 §4.2에서 정의하듯 드래그는 **보조 입력**이며 주 네비게이션 경로는 좌우 핸들(PrevTrigger/NextTrigger)과 키보드입니다.
- `pointerdown` 시 이미지 고스트/텍스트 선택을 막기 위해 Track에 `user-select: none`, 이미지에 `draggable={false}`를 적용하고 필요한 경우 `preventDefault()`를 호출합니다.
...
- **PrevTrigger / NextTrigger**: 좌/우(또는 상/하) 이동을 담당하는 **기본 마우스·키보드 핸들**. 데스크톱에서 주된 네비게이션 수단이며 **기본적으로 항상 렌더링하는 것을 권장**합니다(모바일 전용 UI를 의도할 때만 생략). 네이티브 `<button>`으로 렌더링, `aria-label="Previous slide"` / `"Next slide"` 기본값 제공. 양 끝에 도달하고 `loop=false`이면 `disabled` + `data-disabled`. 클릭 시 `onIndexChange(index ± 1)`과 함께 autoplay 타이머를 리셋합니다.
