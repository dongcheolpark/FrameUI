  ...
  return (
    <article className="page">
      ...
      <Install
        ...
        importStmt={`import { Switch } from "FrameUI";`}
      />

      <Section id="overview" title="Overview">
        <p>
          Switch는 켜짐과 꺼짐, 두 상태 중 하나를 즉시 전환하는 토글
          컨트롤입니다. 알림 수신 여부, 다크 모드, Wi-Fi 같은 설정 페이지의
          on/off 항목이나, 어떤 기능을 활성/비활성하는 자리에서 자주 쓰입니다.
          사용자가 토글을 움직이는 순간 변경이 바로 반영되는 시나리오에
          적합하며, "저장" 버튼을 누르기 전까지 적용을 미뤄야 하는 경우에는 일반
          체크박스가 더 어울립니다.
        </p>
      </Section>
      ...
      <Section id="basic-example" title="Basic example">
        <p>
          제어하지 않는 경우 <code>defaultChecked</code>로 초기 상태를 지정하면
          됩니다.
        </p>
        <Example
      ...
      <Section id="controlled" title="Controlled value">
        <p>
          <code>checked</code>와 <code>onCheckedChange</code>를 함께 넘기면
      ...
      <Section id="disabled" title="Disabled">
        <p>
          비활성화하면 클릭과 키보드 토글이 모두 막히고{" "}
          <code>data-disabled</code>가 적용됩니다.
        </p>
        <Example
          preview={<Switch defaultChecked disabled aria-label="Disabled" />}
          code={disabledCode}
        />
      </Section>
          ...
          rows={[
            {
              prop: "checked",
              type: "boolean",
              description: "Controlled 상태에서 현재 값.",
            },
            {
              prop: "defaultChecked",
              type: "boolean",
              defaultValue: "false",
              description: "Uncontrolled 모드에서 초기 값.",
            },
            {
            ...
            },
            {
              ...
              description:
                "aria-label 등 input 속성을 위임. checked/defaultChecked/onChange/type은 가려져 있습니다.",
            },
          ]}
        />
      </Section>
