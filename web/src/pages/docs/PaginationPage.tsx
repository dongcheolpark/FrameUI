...
function CompactDemo() {
  ...
  return (
    ...
    >
      <Pagination.Prev
        className="demo-pagination-button"
  ...
  return (
    <article className="page">
      ...
      <Section id="overview" title="Overview">
        <p>
          Pagination은 한 번에 다 보여주기엔 너무 많은 데이터를 페이지 단위로
          잘라서 보여주고, 사용자가 페이지를 옮겨다닐 수 있게 해 주는
          네비게이션입니다. 게시판/댓글 목록, 검색 결과, 상품 카탈로그, 관리자
          테이블처럼 수십~수천 건의 항목을 일정한 단위로 끊어 보여줘야 하는
          화면에서 사용됩니다. 현재 페이지 번호, 양 끝/주변 페이지 번호,
          이전·다음 버튼, 그리고 가운데 생략 부호(…)를 함께 제공해 사용자가
          자신이 전체 중 어디쯤에 있는지 한눈에 파악하면서 원하는 페이지로
          빠르게 점프할 수 있습니다.
        </p>
      </Section>
      ...
      <Section id="basic-example" title="Basic example">
        <p>
          <code>totalPages</code> 와 <code>onPageChange</code> 만 주면 끝납니다.
          화살표 끝 페이지에서는 Prev/Next 가 자동으로 disabled 됩니다.
        </p>
      ...
      <Section id="compact" title="Compact (mobile) layout">
        <p>
          모바일에서는 번호 나열을 포기하고 Prev / Next + 현재·전체 텍스트만
          노출하는 것이 보통 더 낫습니다. Compound 로 내부 구조를 직접
          조립하세요.
        </p>
      ...
      <Section id="boundary" title="Sibling and boundary count">
        <p>
          <code>siblingCount</code> 는 현재 페이지 양쪽에 보여줄 번호 수,{" "}
          <code>boundaryCount</code> 는 양 끝에 고정으로 보여줄 번호 수입니다.
          모바일에서는 둘 다 0으로 줄여서 사용하는 패턴이 많습니다.
        </p>
      ...
      <Section id="api" title="Component API">
        ...
        <PropTable
          rows={[
            {
              prop: "page / defaultPage",
              type: "number",
              description: "Controlled / uncontrolled 현재 페이지 (1-based).",
            },
            {
              prop: "onPageChange",
              type: "(page: number) => void",
              description:
                "페이지 변경 콜백. 1-based page 번호 하나만 전달됩니다.",
            },
            {
              prop: "totalPages",
              type: "number (필수)",
              description:
                "전체 페이지 수. 0 또는 1이면 Prev/Next 모두 disabled.",
            },
            {
              prop: "siblingCount",
              type: "number",
              defaultValue: "1",
              description: "현재 페이지 양쪽에 보여줄 번호 수.",
            },
            {
              prop: "boundaryCount",
              type: "number",
              defaultValue: "1",
              description: "양 끝에 고정으로 보여줄 번호 수.",
            },
            {
              prop: "disabled",
              type: "boolean",
              defaultValue: "false",
              description: "전체 비활성화. 모든 서브 컴포넌트에 전파.",
            },
            {
              prop: "aria-label",
              type: "string",
              defaultValue: `"Pagination"`,
              description: "nav 랜드마크의 접근성 이름.",
            },
          ]}
        />
        ...
        <p>
          <code>Item</code> 은 <code>value: number</code> 를 받고 현재 페이지와
          비교해 자동으로 <code>aria-current="page"</code> 와{" "}
          <code>data-state="active|inactive"</code> 를 부여합니다.{" "}
          <code>Prev / Next / First / Last</code> 는 경계에서 자동으로 disabled
          됩니다. 아이콘만 사용하면 기본 <code>aria-label</code>(
          <code>"Previous page"</code> 등)이 자동으로 적용됩니다.
        </p>
      </Section>
    </article>
  );
